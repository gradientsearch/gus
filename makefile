...
dev-logs:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(GUS_APP) --all-containers=true -f --tail=100 --max-log-requests=6 | go run api/tooling/logfmt/main.go -service=$(GUS_APP)
...
dev-logs-init:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(GUS_APP) -f --tail=100 -c init-migrate-seed
...
dev-services-delete:
	kustomize build zarf/k8s/dev/gus | kubectl delete -f -
...
dev-describe-replicaset:
	kubectl get rs
	kubectl describe rs --namespace=$(NAMESPACE) -l app=$(GUS_APP)
...
dev-shell:
	kubectl exec --namespace=$(NAMESPACE) -it $(shell kubectl get pods --namespace=$(NAMESPACE) | grep gus | cut -c1-26) --container $(GUS_APP) -- /bin/sh
...
migrate:
	export GUS_DB_HOST=localhost; go run api/tooling/admin/main.go migrate

seed: migrate
	export GUS_DB_HOST=localhost; go run api/tooling/admin/main.go seed
...
token-gen:
	export GUS_DB_HOST=localhost; go run api/tooling/admin/main.go gentoken 5cf37266-3473-4006-984f-9325122678b7 54bb2165-71e1-41a6-af3e-7da4a0e1e2c1
...
messages:
	curl -il \
	-H "Authorization: Bearer ${TOKEN}" "http://localhost:3000/v1/messages" -d  '{"conversationID": "53c1d1dd-8b23-49c7-a0ce-eb6a2747c5a4", "messages": [{"id": "$(shell uuid | tr -d '\n')", "role": "user", "content": "Message 4"}], "parentMessageID": "00000000-0000-0000-0000-000000000000"}'
...
run:
	go run api/services/gus/main.go | go run api/tooling/logfmt/main.go

run-help:
	go run api/services/gus/main.go --help | go run api/tooling/logfmt/main.go
...
talk-logs:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(GUS_APP) --all-containers=true -f --tail=100 --max-log-requests=6

talk-logs-cpu:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(GUS_APP) --all-containers=true -f --tail=100 --max-log-requests=6 | grep SCHED

talk-logs-mem:
	kubectl logs --namespace=$(NAMESPACE) -l app=$(GUS_APP) --all-containers=true -f --tail=100 --max-log-requests=6 | grep "ms clock"
...
help:
	...
	@echo "  gus                   Build the gus container"
	...
	@echo "  dev-logs                Show the logs for the gus service"
	...
	@echo "  dev-describe-gus      Show the gus pod details"
