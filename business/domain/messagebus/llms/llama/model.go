...
func busToLlmMessages(messages []messagebus.Message) []Message {
...
func llmToBusMessages(msg Message) (messagebus.Message, error) {
	var m messagebus.Message
	m.ID = uuid.New()
	m.Content = msg.Content
	r, err := messagebus.ParseLlmRoles(msg.Role)
	if err != nil {
		return messagebus.Message{}, fmt.Errorf("unexpected role: %s", msg.Role)
	}
	m.Role = r
