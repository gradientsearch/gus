...
func toBusConversation(dbCon []conversationMessages) (messagebus.Conversation, error) {
	if len(dbCon) < 1 {
		return messagebus.Conversation{}, fmt.Errorf("db: conversation not found error")
	}

	busConvo := messagebus.Conversation{}

	for _, m := range dbCon {
		bm := messagebus.Message{}
		bm.ID = m.MessageID
		bm.Content = m.message.Content
		bm.Role = messagebus.NewRole(m.Role)
		bm.Order = m.Order
...
func toDbConversation(busCon messagebus.Conversation) conversation {
...
func toDbMessages(busMsgs []messagebus.Message) []message {
